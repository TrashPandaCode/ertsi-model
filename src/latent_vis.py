...
#model = ReverbCNN.load_from_checkpoint("checkpoints/synth_only/exV3-reverbcnn-synth-only-epoch=07-val_loss=0.3029.ckpt")

# Option 2: Load from saved state dict (alternative approach)
model = ReverbCNN(num_frequencies=len(FREQS))
model.load_state_dict(torch.load("output/exV3-reverbcnn_synth_only.pt"))
model.eval()
