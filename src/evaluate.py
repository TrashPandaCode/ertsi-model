...
def evaluate(model_path="output/reverbcnn.pt", data_dir="data", batch_size=32):
    set_seeds(42)
    ...
    # Load the dataset
    dataset = ReverbRoomDataset(data_dir, freqs=freqs)
    ...
    for i, freq in enumerate(freqs):
        ...
        plt.axvline(mean_error, color='r', linestyle='--', label=f'Mean: {mean_error:.3f}s')
        plt.axvline(mean_error + std_error, color='g', linestyle=':', label=f'Std: {std_error:.3f}s')
