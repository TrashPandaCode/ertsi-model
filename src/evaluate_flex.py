...
from model_flex import ReverbCNN  # Changed from model import
...
def evaluate(
    model_path="output/exV6-reverbcnn_resnet50_places365.pt",  # Updated default path
    data_dir="data/test/real",
    batch_size=32,
    backbone="resnet50_places365",  # Added backbone parameter
):
    set_seeds(42)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")
    print(f"Using backbone: {backbone}")  # Added backbone info
    ...
    # Updated model initialization with backbone parameter
    model = ReverbCNN(num_frequencies=len(freqs), backbone=backbone)
    ...
    # Create evaluation directory with backbone name
    eval_dir = f"evaluation/{backbone}"
    ...
    plt.tight_layout()
    plt.savefig(f"{eval_dir}/plots/prediction_with_uncertainty.png")
    ...
    plt.tight_layout()
    plt.savefig(f"{eval_dir}/plots/rt60_heatmap_density.png")
    ...
    plt.tight_layout()
    plt.savefig(f"{eval_dir}/plots/rt60_kde_heatmap.png")
...
def evaluate_all_backbones():
    """Evaluate all backbone architectures for comparison"""
    backbones = [
        "resnet50_places365",
        "efficientnet_b4",
        "convnext_base",
    ...
    for backbone in backbones:
        ...
        try:
            model_path = f"output/comparison-reverbcnn_{backbone}.pt"
            if not os.path.exists(model_path):
                print(f"Model file not found: {model_path}")
                continue

            metrics = evaluate(model_path=model_path, backbone=backbone)
    ...
    if results:
        os.makedirs("evaluation/comparison", exist_ok=True)
        with open("evaluation/comparison/all_backbones_metrics.json", "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="output/exV6-reverbcnn_resnet50_places365.pt",
    ...
    parser.add_argument(
        "--all", action="store_true", help="Evaluate all backbone architectures"
    )

    args = parser.parse_args()

    if args.all:
        evaluate_all_backbones()
    else:
        evaluate(model_path=args.model_path, backbone=args.backbone)
