...
def evaluate(
    model_path="output/ensemble_reverbcnn.pt",  # Changed default path
    ...
    ensemble_models = []
    for i in range(3):  # Assuming 3 models in ensemble
        model = ImprovedReverbCNN(num_frequencies=len(freqs))
        individual_path = f"output/improved_reverbcnn_final_ensemble_{i}.pt"
        model.load_state_dict(torch.load(individual_path, map_location=device))
        model.to(device)
        ensemble_models.append(model)

    # Create ensemble model
    model = EnsembleReverbCNN(ensemble_models)
    model.to(device)
    model.eval()
    ...
    with torch.no_grad():
        for inputs, targets in dataloader:
            ...
            outputs, _ = model(inputs)  # Ensemble returns (mean, std)
