...
def train():
    ...
    params = {
        ...
        "synth_model_out": "output/exV2-reverbcnn_synth.pt",  # Checkpoint after synthetic training
        "final_model_out": "output/exV2-reverbcnn.pt",  # Final fine-tuned model
    }

    os.makedirs(os.path.dirname(params["synth_model_out"]), exist_ok=True)

    synth_train = ReverbRoomDataset(
        "data/train/synth/non-hybrid",
        freqs=params["freqs"],
        augment=True,
    )
    ...
    checkpoint_callback_synth = ModelCheckpoint(
        ...
        filename="exV2-reverbcnn-synth-{epoch:02d}-{val_loss:.4f}",
        save_top_k=3,
        monitor="val_loss",
        mode="min",
    )
    ...
    logger_synth = TensorBoardLogger("logs", name="exV2-reverbcnn_synth")
    ...
    checkpoint_callback_real = ModelCheckpoint(
        ...
        filename="exV2-reverbcnn-real-{epoch:02d}-{val_loss:.4f}",
        save_top_k=3,
        monitor="val_loss",
        mode="min",
    )
    ...
    logger_real = TensorBoardLogger("logs", name="exV2-reverbcnn_real_finetune")
